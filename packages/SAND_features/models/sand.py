...
class Sand(nn.Module):
    def __init__(self, n_dims):
        ...
        self.final = nn.Conv2d(
            32, self.n_dims, kernel_size=1, padding=0, stride=1, bias=False)
    ...
    def load_ckpt(self, ckpt_file):
        ...
        try:
            ...
            state = {k.replace('branch.', ''): v for k,
                     v in ckpt['model'].items()}
    ...
    def _make_layer(self, block, out_channels, blocks, stride, pad, dilation):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.in_channels, out_channels, 1, None, pad, dilation))
    ...
    def forward(self, x):
        ...
        # Decoder
        features = (output_raw, output_skip, output_branch4,
                    output_branch3, output_branch2, output_branch1)
        output_feature = torch.cat(features, 1)

        output_feature = F.interpolate(
            output_feature, skip.size()[-2:], mode='bilinear', align_corners=True)
        output_feature = self.lastconv(output_feature).add_(skip)

        output_feature = F.interpolate(
            output_feature, x.size()[-2:], mode='bilinear', align_corners=True)
